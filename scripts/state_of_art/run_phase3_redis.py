#!/usr/bin/env python3
"""Run the Redis gate and emit a commit-bound Phase 3 evidence envelope."""
...
try:
    from scripts.phase11 import redis_runtime_gate
...
DEFAULT_OUTPUT = ".runtime/phase-3/redis-runtime-evidence.json"
RAW_OUTPUT = ".runtime/phase-3/raw/redis-runtime-gate.json"
CAPABILITY_ID = "P0-06"
...
def run(root: Path = ROOT, *, output: str = DEFAULT_OUTPUT, argv: Sequence[str] = ()) -> dict[str, Any]:
    return run_gate_adapter(
        root,
        redis_runtime_gate.main,
        capability_id=CAPABILITY_ID,
        output=output,
        raw_output=RAW_OUTPUT,
        environment="phase3-redis-local-disposable",
        procedure="scripts/phase11/redis_runtime_gate.py against an explicitly supplied disposable Redis URL",
        argv=argv,
