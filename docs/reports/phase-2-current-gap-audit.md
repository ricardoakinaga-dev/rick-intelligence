...
| Citation verification | `PARTIAL` | `packages/evidence/`, `packages/professor/`, decision path | Forged/stale/cross-tenant/wrong-checksum negative tests exist locally | No claim-to-text evaluation over approved corpus | Citation existence is stronger than syntax but claim support metrics are missing | P1 | `citation_precision`, `citation_recall`, `citation_completeness` and `unsupported_claim_rate` run on versioned approved data |
