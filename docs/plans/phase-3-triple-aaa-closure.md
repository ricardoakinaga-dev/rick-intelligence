...
## 46. Local object-envelope JSON closure — 2026-09-10

Source implementation candidate `1816748cf44dee8e48b16d718142e2ed8550ca51`
(tree `2716c60e59134c8b1e8f3c1e4824b01d0a41bfa9`) closes the local object
envelope boundary. The fixed 4 KiB header now writes finite canonical JSON and
rejects non-finite constants, duplicate keys, recursion/encoding failures and
malformed metadata during reads, failing closed before scope, key, size or
checksum metadata is trusted. The regression covers a `NaN` field and a
duplicate header key.

The storage suite passes **26** tests; compilation and `git diff --check` pass.
This is local object-store evidence only. It does not prove the live
...
Source candidate `e62afddd2124ae71ef9b05f6ff50f6db226aec6b` (tree
`847aa5a8a93243971cd787961d7c4014f8b7e329`) closes the remaining ambiguity
in the local SQLite queue, legacy PostgreSQL queue and canonical PostgreSQL
job adapter. Their persisted JSON decoders now reject duplicate object keys in
addition to the existing byte, finite-value, recursive and malformed-input
controls; a corrupt row cannot change a job field by relying on the last
duplicate value.
...
Source candidate `bb238b1b4165dc4e580d7d9aa294703ff9c41783` (tree
`30f073d27b71b8e2e57596815d74725307cd15c8`) closes the remaining ambiguity in
the hermetic SQLite vector read-model. The bounded persisted JSON decoder now
rejects duplicate object keys before vector or payload projection; a payload
whose duplicate key would otherwise select the last value fails closed while
the canonical checksum remains valid.
