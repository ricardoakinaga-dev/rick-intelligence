...
| 3.3 PostgreSQL + workers | Migrations, queue, leases, fencing, crash matrix | No fake DB or single-process substitution | Execute existing real gates and add missing failure assertions | SQL/migration, concurrency and recovery suite | Real DSN, two workers, attempt/outbox/audit evidence | Roll-forward or isolated restore | DB/runtime critics | `POSTGRES_WORKERS_PASS` |
...
The current next action is to implement Slice 3.1 after the audit artifacts
and exact prompt copy are present, then run a fresh control/release critic.
